function [] = FMCW_sim_hand(hands, fout)
...
hands = hands/5; % normalize for lower max_vel
num_tar = size(hands,1);
durationx = size(hands,3)/fps_skel;
numChirps = floor(durationx*NPpF*(1/frameDuration));
tar_loc = zeros(num_tar, size(hands,2), numChirps);
% vel_hist = zeros(num_tar, size(hands,2));

for t = 1:num_tar
    for i = 1:size(hands,2)
        tar_loc(t,i,:) = spline(1:size(hands,3), squeeze(hands(t,i,:)), linspace(1,size(hands,3), numChirps));      
%         tar_loc(t,i,:) = makima(1:size(hands,3), squeeze(hands(t,i,:)), linspace(1,size(hands,3), numChirps));      
...
% figure('visible','off')
% cnt = 1;
% for n = 1:ceil(numChirps/size(hands,3)):numChirps
